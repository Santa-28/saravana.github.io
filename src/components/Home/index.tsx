...
//           </p>
//           <h1>
//             {currentLang === 'ta' ? 'சரவணகுமார்' : 'Saravanakumar'}
...
//             <Typewriter
//               options={{
//                 strings: [currentLang === 'ta' ? 'டெவலப்பர் | புரோகிராமர் | ஃப்ரீலான்சர்' : 'Developer | Programmer | Freelancer'],
//                 autoStart: true,
//                 loop: true,
//               }}
//             />
...
export function HomeHero() {
  ...
  async function handleGenerate() {
    ...
    try {
      if (!GOOGLE_FORM_ACTION_URL2) return

      const formData = new FormData()
      formData.append(NAME_ENTRY_ID2 || '', userName)

      await fetch(GOOGLE_FORM_ACTION_URL2, {
        method: 'POST',
        body: formData,
        mode: 'no-cors',
      })
  ...
  return (
    <Container>
      <Content>
        <HomeText>
          <p><span>👋🏻</span> Hello, my name is</p>
          <h1>
            Saravanakumar
            <span className="animation">
              <Image width={200} height={200} src="/vectors/triangle.svg" alt="triangle" />
            </span>
          </h1>
          <h2>
            <Typewriter
          ...
          <div className="button">
            ...
            {/* Your Generate Card button and modal can follow here */}
          </div>
        </HomeText>

        {showConfetti && (
          <Confetti width={window.innerWidth} height={window.innerHeight} numberOfPieces={250} gravity={0.5} wind={0.05} />
        )}

        <ImgHome>
          <img className="home-img" src="/home.png" alt="Image Home" />
          <div className="code">
            <Image width={150} height={50} src="/vectors/dino.svg" alt="dino" />
          </div>
        </ImgHome>
      </Content>
    </Container>
